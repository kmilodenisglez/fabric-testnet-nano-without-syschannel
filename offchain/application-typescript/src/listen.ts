...
class BlockProcessor {
    ...
    async process(): Promise<void> {
        ...
        const transactions = this.#getNewTransactions()
...
class TransactionProcessor {
    ...
    async process(): Promise<void> {
        ...
        const writes = this.#getWrites(transactionId);
        if (writes.length === 0) {
    ...
    #getWrites(txID: string): Write[] {
        const channelHeader = this.#transaction.getChannelHeader();
        const channelName = channelHeader.getChannelId();
        let ts_ = channelHeader?.getTimestamp();
        let timestamp = (ts_ === undefined ? 0 : ts_.toDate().getTime())
        let dateInSec = (ts_ === undefined ? "" : ts_.toDate().toString())
            ...
            .flatMap(readWriteSet => {
                ...
                return readWriteSet.getReadWriteSet().getWritesList().map(write => {
                    return {
                        ...
                        isValid: this.#transaction.isValid(),
